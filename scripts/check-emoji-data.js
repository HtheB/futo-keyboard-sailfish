...
const searchSourcePath = path.join(root, "layouts", "FutoEmojiSearchData.js");
...
vm.runInContext(fs.readFileSync(searchSourcePath, "utf8"), searchContext,
                { filename: searchSourcePath });
...
assert(searchContext.allEntries.length === entries.length,
...
for (const query of ["happy", "dog", "pizza", "train", "football", "light bulb", "heart", "flag netherlands"])
    assert(searchContext.search(query, ["EN"]).length > 0,
           "Search returned no matches for: " + query);

const englishBeer = searchContext.search("beer", ["EN"]);
const dutchBeer = searchContext.search("beer", ["NL"]);
const bilingualBeer = searchContext.search("beer", ["EN", "NL"]);
...
const turkishBear = searchContext.search("ayı", ["TR"]);
const turkishUpperBear = searchContext.search("AYI", ["TR"]);
...
assert(searchContext.standardFold("İ") === "i",
       "Standard İ folding retained a combining dot");
assert(searchContext.turkishFold("Iİıi") === "ıiıi",
